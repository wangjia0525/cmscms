...
App.controller('unauditedArticleCtrl',function($scope, $http,Notify,$timeout,$location, $cookieStore,yulanService, ModalService,articleFactory,mdFactory){
    //更改业务状态
    $scope.updateClearingCallback=function(data){
        $scope.fetchData();
        Notify.alert();
    }
    $scope.operate=function(id,status,userId,title){
        $scope.id=id;
        $scope.userId=userId;
        $scope.type=status;
        $scope.confirm=function(type,instructions){
            console.log(type)
            $scope.getParams2 = function(){
                if(type==2){
                    return {
                        articleId:id,
                        userId:$scope.userId,
                        instructions:'你的文章《'+title+"》通过了审核，预计24小时内发布。",
                        status:  type
                    }
                }else{
                    return {
                        ...
                        userId:$scope.userId,
                        status:  type,
                        instructions:instructions||"您的文章《"+title+"》未能通过审核，请打开网站查看修改。"
                    }
                }
    ...
    $scope.reviewStatus = [
        {_index_: 1, _value_: "全部"},
        {_index_: 4, _value_: "未审核"},
        {_index_: 3, _value_: "审核未通过"}
    ];
    $scope.reviewStatusSelectValue = $scope.reviewStatus[0];
    /*
     时间框
     * */
    ...
    $scope.getParams = function () {
        ...
        return {
            ...
            isShow:3,
            reviewStatus:$scope.reviewStatusSelectValue._index_,
